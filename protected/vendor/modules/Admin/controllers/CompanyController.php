...
class CompanyController extends Controller {
    ...
    public function accessRules() {
        return array(
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array('create', 'update', 'index', 'view', 'admin', 'delete','activate','viewRequests'),
    ...
    public function actionActivate($id) {
        $co=Company::model()->findByPk($id);
        $u=Users::model()->findByPk($co->user_id);
        $u->activated=1;
$b='Ο λαγαριασμός σας ενεργοποιήθηκε από την ομάδα της e-position. Μπορείτε να εισέλθετε με τους κωδικούς σας.';
        
        $this->SendMail($id,$b);
        if($u->save()){
	   
          $this->redirect(array('view', 'id' => $co->id));
